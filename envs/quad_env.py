...
class quadrotor:
    ...
    def rotateBFtoGF(self, PHI, THETA, PSI, X, Y, Z):
        X_ = cos(PSI) * cos(THETA) * X + (cos(PSI) * sin(PHI) * sin(THETA) - cos(PHI) * sin(PSI)) * Y + (
                    cos(PHI) * cos(PSI) * sin(THETA) + sin(PHI) * sin(PSI)) * Z
        Y_ = sin(PSI) * cos(THETA) * X + (sin(PSI) * sin(PHI) * sin(THETA) + cos(PHI) * cos(PSI)) * Y + (
                    cos(PHI) * sin(PSI) * sin(THETA) - cos(PSI) * sin(PHI)) * Z
    ...
    def update_point(self, n):
        ...
        self.PID_rate()
        self.quad_motor_speed()
        state = self.step(self.state, self.input_vector)

        # length of the arm of the quadrotor is 0.5m
        (x_bf, y_bf, z_bf) = self.rotateGFtoBF(state[0], state[1], state[2], state[9], state[10], state[11])
        (x_bl1, y_bl1, z_bl1) = self.rotateBFtoGF(state[0], state[1], state[2], x_bf, y_bf + 0.25, z_bf)
        (x_bl2, y_bl2, z_bl2) = self.rotateBFtoGF(state[0], state[1], state[2], x_bf + 0.25, y_bf, z_bf)
        (x_bl3, y_bl3, z_bl3) = self.rotateBFtoGF(state[0], state[1], state[2], x_bf, y_bf - 0.25, z_bf)
        (x_bl4, y_bl4, z_bl4) = self.rotateBFtoGF(state[0], state[1], state[2], x_bf - 0.25, y_bf, z_bf)
...
def main():
    qd = quadrotor(Ts = 1.0/16.0, USE_PWM = 0, USE_PID = 1)
    qd.des_xyz(1.0, 1.0, 1.0)
