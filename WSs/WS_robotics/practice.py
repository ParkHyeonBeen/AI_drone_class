...
for i in range(0, n):
    ...
    mat_inertia[0, 0, i] = (M1 + M2) * L1 ** 2 + M2 * L2 ** 2 + 2 * M2 * L1 * L2 * cos(Theta2_desire[i])
    mat_inertia[1, 0, i] = M2 * L2 ** 2 + M2 * L1 * L2 * cos(Theta2_desire[i])
    mat_inertia[0, 1, i] = M2 * L2 ** 2 + M2 * L1 * L2 * cos(Theta2_desire[i])
    mat_inertia[1, 1, i] = M2 * L2**2

    mat_cori[0, 0, i] = -M2 * L1 * L2 * (2 * Theta1_d_desire * Theta2_d_desire + Theta2_d_desire ** 2) * sin(
        Theta2_d_desire)
    mat_cori[1, 0, i] = M2 * L1 * L2 * Theta1_d_desire ** 2 * sin(Theta2_d_desire)

    mat_grav[0, 0, i] = (M1 + M2) * L1 * cos(Theta2_desire(i)) + M2 * L2 * cos(Theta1_desire(i) + Theta2_desire(i))
    mat_grav[1, 0, i] = M2 * L2 * cos(Theta1_desire(i) + Theta2_desire(i))

    mat_torque[i] = mat_inertia*state_dd
'''
for i in range(n):
