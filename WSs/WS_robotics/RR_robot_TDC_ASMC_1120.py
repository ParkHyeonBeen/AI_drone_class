...
Estimation_System = np.zeros((n, 2, 1))
...
mat_inertia_const = np.matrix('1 -2; -2 5')
...
for i in range(0, n):
    ...
    Estimation_System[i + 1, :, :] = state_dd_current[i, :, :] - np.dot(mat_inertia_const, Torque_CMD[i, :, :])
    ...
    estimation_tmp = state_CMD[i, :, :] - Estimation_System[i, :, :]
    Torque_CMD[i, :, :] = np.dot(mat_inertia_const, estimation_tmp)
